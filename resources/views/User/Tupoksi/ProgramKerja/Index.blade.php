  ...
  <section class="py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4">
      ...
      <div class="mb-12 text-center">
        <h2 class="text-3xl font-extrabold text-green-800 mb-2">Program Kerja</h2>
        <p class="text-gray-600 text-sm">Temukan informasi terbaru seputar kegiatan, dakwah, dan kabar menarik lainnya.</p>
      </div>

      <!-- Search Bar -->
      <form method="GET" action="{{ route('Index.ProgramKerja') }}" class="mb-10 max-w-2xl mx-auto">
        <div class="relative">
      ...
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <a href="{{ route('Show.ProgramKerja', $proker->title) }}">
          <div class="bg-white rounded-xl overflow-hidden shadow hover:shadow-lg transition-all duration-300 group">
            <!-- Gambar -->
            <div class="h-48 overflow-hidden">
              <img src="{{ $proker->image ? asset('storage/posts/' . $proker->image) : 'https://via.placeholder.com/600x300?text=No+Image' }}"
                alt="{{ $proker->title }}"
            ...
            <div class="p-5 space-y-3">
              <h4 class="text-lg font-semibold text-gray-900 leading-snug hover:text-green-600 transition">
                {{ Str::limit($proker->title, 90) }}
              </h4>
              <p class="text-sm text-gray-600 leading-relaxed">
                {{ Str::limit(strip_tags($proker->content), 80) }}
              </p>

              <!-- Meta Info -->
              <div class="flex items-center justify-between text-xs text-gray-500 mt-3">
                <div class="flex items-center gap-2">
                  <span>{{ $proker->user->name ?? 'Unknown' }}</span>
                </div>
                <span><i class="far fa-clock mr-1"></i>{{ $proker->created_at->format('d M Y') }}</span>
              </div>
            </div>
          </div>
        </a>
      ...
      <div class="mt-12 text-center">
        {{ $prokers->links('vendor.pagination.tailwind') }}
      </div>
      @endif
