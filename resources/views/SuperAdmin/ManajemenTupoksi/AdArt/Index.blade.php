...
<div class="bg-white shadow-md rounded-xl p-8 mb-10 border border-green-100">
  ...
  <form action="{{ route('Store.AdArt.SA') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
          ...
          @if($AdArtItems)
            <tr class="border-b hover:bg-green-50">
              ...
              <td class="px-4 py-2 space-x-2">
                <form id="deleteForm" action="{{ route('Delete.AdArt.SA', $AdArtItems->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
