<!-- Sidebar Desktop -->
<aside class="w-64 bg-white border-r border-slate-200 hidden md:flex flex-col z-30 shadow-[4px_0_24px_rgba(0,0,0,0.02)] transition-all">
  <!-- Header -->
  ...
  <!-- Navigation -->
  <nav class="flex-1 mt-6 px-4 py-2 space-y-1.5 text-sm font-medium overflow-y-auto custom-scrollbar">
    ...
    <a href="{{ route('Index.Dashboard.SA') }}"
       ...
       @endif
      <div class="flex items-center justify-center w-8 transition-transform group-hover:scale-110">
    ...
    <div class="tupoksi-wrapper">
      <button type="button"
        class="tupoksi-toggle w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all duration-200 relative group overflow-hidden focus:outline-none
      ...
      <div class="tupoksi-submenu ml-11 mt-1 space-y-1 overflow-hidden transition-all duration-300 {{ request()->routeIs('Index.SK.SA') || request()->routeIs('Index.StrukturOrganisasi.SA') || request()->routeIs('Index.AdArt.SA') || request()->routeIs('Index.ProgramKerja.SA') ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0' }}">
  ...
  <div class="p-4 border-t border-slate-100 bg-slate-50/50">
    <form method="POST" action="{{ route('logout.SA') }}">
      ...
      <button type="submit"
        ...
        <i class="fas fa-sign-out-alt group-hover:-translate-x-1 transition-transform"></i> <span class="font-medium">Keluar</span>
      </button>
    </form>
  </div>
</aside>
