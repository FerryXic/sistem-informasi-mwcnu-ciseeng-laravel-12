<!-- Sidebar Desktop -->
<aside class="w-64 bg-white shadow-md border-r border-green-100 hidden md:flex flex-col" id="adminSidebar">
  <!-- Header -->
  ...
  <nav class="flex-1 mt-4 px-4 py-2 space-y-1 text-sm text-green-900 font-medium">
    ...
    <a href="{{ route('Index.ManajemenPost.A') }}"
       class="flex items-center gap-3 px-4 py-2 rounded-lg transition hover:bg-green-50 hover:text-green-700
       {{ request()->routeIs('Index.ManajemenPost.A') ? 'bg-green-100 text-green-800 font-semibold' : '' }}">
      <i class="fas fa-thumbtack text-green-500 w-5"></i> Manajemen Post
    </a>

    <!-- Manajemen Tupoksi Dropdown -->
    <div class="tupoksi-wrapper">
      <button type="button"
        ...
        {{ request()->routeIs('Index.SK.A') || request()->routeIs('Index.StrukturOrganisasi.A') || request()->routeIs('Index.AdArt.A') || request()->routeIs('Index.ProgramKerja.A') ? 'bg-green-100 text-green-800 font-semibold' : '' }}">
      ...
      <div class="tupoksi-submenu hidden ml-6 mt-2 space-y-1 text-sm text-green-700">
        <a href="{{ route('Index.SK.A') }}"
           ...
           {{ request()->routeIs('Index.SK.A') ? 'text-green-800 font-semibold' : '' }}">
          <i class="fas fa-file-signature mr-2"></i> SK
        </a>
        <a href="{{ route('Index.StrukturOrganisasi.A') }}"
           class="block px-3 py-1 rounded hover:bg-green-50 transition
           {{ request()->routeIs('Index.StrukturOrganisasi.A') ? 'text-green-800 font-semibold' : '' }}">
    ...
    <a href="{{ route('Index.ManajemenSurat.A') }}"
       class="flex items-center gap-3 px-4 py-2 rounded-lg transition hover:bg-green-50 hover:text-green-700
       {{ request()->routeIs('Index.ManajemenSurat.A') ? 'bg-green-100 text-green-800 font-semibold' : '' }}">
    ...
    <a href="{{ route('Index.Profile.A') }}"
       class="flex items-center gap-3 px-4 py-2 rounded-lg transition hover:bg-green-50 hover:text-green-700
       {{ request()->routeIs('Index.Profile.A') ? 'bg-green-100 text-green-800 font-semibold' : '' }}">
      <i class="fas fa-tools text-green-500 w-5"></i> Pengaturan
    </a>
  </nav>

  <!-- Logout -->
  <form method="POST" action="{{ route('logout.A') }}" class="px-6 py-4 border-t border-green-100">
    @csrf
    <button type="submit"
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    @if(request()->routeIs('Index.SK.A') || request()->routeIs('Index.StrukturOrganisasi.A') || request()->routeIs('Index.AdArt.A') || request()->routeIs('Index.ProgramKerja.A'))
